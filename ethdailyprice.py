...
def get_daily_prices(start_date, vs_currency='usd'):
    ...
    url = f'{base_url}/coins/ethereum/market_chart'
...
def main():
    ...
    print(f"Fetching daily ETH prices from {start_date} to today...")
    daily_prices = get_daily_prices(start_date)
    
    if daily_prices:
        # Generate filename with current timestamp
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'eth_daily_prices_{timestamp}.json'
